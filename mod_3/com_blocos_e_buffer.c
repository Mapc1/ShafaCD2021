...
void PontoShafa (char* buffer, FILE *fptr, char **freq, int tam_buffer)
{
    ...
    for (atual = 0; atual < tam_buffer; atual++)
        ...
        int tam;
            for (tam = 0; bitsTalvez[tam] != '9'; tam++);
 ...
 /*      
    ...
    char array_final[numListas];
        fprintf(fptr, "@%d@",numListas );
    converteListaArray(array_final, list);
    fwrite(array_final , numListas , sizeof(char) , fptr );
    printf("\n AAA Olá %s \n", array_final);    
}
...
int main () {
    ...
    int i;
    //char *c;
	if ((fp = fopen("aaa.txt.rle.cod", "r+")) == NULL)  printf("Error! opening file");         // Program exits if file pointer returns NULL.
    if ((fptr = fopen("aaaM.shaf","wb")) == NULL){
       printf("Error! opening file");
    }

    if ((fpOrigi = fopen("aaa.txt.rle","rb")) == NULL){
       printf("Error! opening file");
    }
    else {
